...
public class GoodsPlaceCarItemAdapter extends BaseExpandableListAdapter {
    ...
    public GoodsPlaceCarItemAdapter(Context context, List<ShopListBean2> list, ShopCarBean shopCarBean, ShopCarMoneyOrNum sh, String ProUserId) {
...
//        if (price >= startPrice || num == itemList.size()) {
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
        ...
        if (childPosition == list.get(groupPosition).getItemList()
                .size() - 1) {
        ...
        switch (integer) {
            ...
            case 2://下架
                ...
                childViewHolder.tvUnderShelf.setVisibility(View.VISIBLE);
                childViewHolder.ivNoGoods.setVisibility(View.VISIBLE);
                childViewHolder.tvOldPrice.setVisibility(View.GONE);
                Glide.with(context).load(shopCarBean.getOffShelveImage());
            ...
            case 3://无货
                ...
                childViewHolder.ivNoGoods.setVisibility(View.VISIBLE);
                childViewHolder.tvOldPrice.setVisibility(View.GONE);
                Glide.with(context).load(shopCarBean.getOutOfStockImage());
        ...
        childViewHolder.avGoodsNum.setAvNumListener(new AvNumListener() {
            ...
            public void num(String number) {
                ...
                commitToService(shopInfo.getShopId(), goodsBean.getCartId(), number, goodsBean.getActivityId());//购物车网络数量操作
                notifyDataSetChanged();
            }
        });
...
//                it.putExtra("formShopCar", "formActiveCar");
//                it.putExtra("Shopid", GlobalApplication.getInstance().getPerson(context).getUnitId());
//                it.putExtra("BUserId", ProUserId);
//                it.putExtra("ActivityId", goodsBean.getActivityId());
//                context.startActivity(it);
    ...
    //代下单购物车数量改变
    private void commitToService(String shopId, String cardId, String number, String ActivityId) {
...
//        hashMap.put("SkuId", skuId);
        ...
        hashMap.put("ProUserId", ProUserId);//买家UserId(代下单的时候需要传入买家的UserId)
        hashMap.put("IsReplaceOrder", AllConstant.IsReplaceOrder);//是否代下单列表（1表示代下单，其它为正常商品列表）(V2.0)
    ...
    static class GroupViewHolder {
        ...
        @Bind(R.id.iv_shop)
        ImageView ivShop;
        @Bind(R.id.iv_join_goods)
        ImageView ivJoinGoods;
